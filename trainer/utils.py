...
class CTCLabelConverter(object):
    ...
    def decode(self, preds_index, preds_size):
        blank_index = len(self.character) # Blank is typically the last index
        preds_str = []

        for i in range(preds_index.size(0)):
            pred = preds_index[i].cpu().tolist() # Convert to list for easier processing
            decoded = []
            prev = None
            ...
            text = ''.join([self.character[idx] for idx in decoded if idx < len(self.character)])
